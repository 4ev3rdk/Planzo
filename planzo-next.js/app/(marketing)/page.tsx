import Link from "next/link";
import localFont from "next/font/local";
import { Poppins } from "next/font/google";
import { Medal } from "lucide-react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";


const headingFont = localFont({ src: "../../public/fonts/CalSans-Regular.woff2" });
const textFont = Poppins({
    subsets: ["latin"],
    weight: ["100", "200", "300", "400", "500", "600", "700", "800", "900"]
});


const MarketingPage = () => {
    return (<div className="flex items-center justify-center  flex-col">

        <div className={cn("flex items-center justify-center flex-col", headingFont.className)} >

            <div className="bg-amber-100 mb-4 flex items-center  border shadow-sm p-4 text-amber-700 rounded-full uppercase">
                <Medal className="h-6 w-6 mr-2" />
                No 1 Project Management
            </div>
            <h1 className=" text-3xl md:text-6xl text-neutral-800 mb-6">Planzo helps team move</h1>

            <div className="text-3xl md:text-6xl bg-gradient-to-r from-fuchsia-600 to-pink-600 text-white px-4 p-2 pb-4 rounded-md w-fit">
                Work Forward
            </div>
        </div>
        <div className={cn("text-sm md:text-xl mt-4 text-neutral-400 max-w-xs md:max-w-2xl text-center mx-auto ", textFont.className)}>
            Collaborate,Manage Project and reach new Productivity peeks. From high rises to home office, the way yout team works is unique - accomplish all with Planzo
        </div>
        <Button className="mt-4" size="lg" asChild >
            <Link href="/sign-up">Get Planzo Free</Link>
        </Button>


    </div >
    );
};
export default MarketingPage;